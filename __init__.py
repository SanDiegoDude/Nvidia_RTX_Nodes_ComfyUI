...
SAME_RESOLUTION_MODES = {"Denoise", "Deblur"}
...
class RTXVideoSuperResolution(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input(
                    "mode",
                    options=["Upscale", "Denoise", "Deblur", "High-Bitrate Upscale"],
                    default="Upscale",
                    tooltip="Upscale: AI upscaling for compressed/general sources. "
                            "Denoise: same-resolution noise and compression artifact removal. "
                            "Deblur: same-resolution sharpening for soft or motion-blurred footage. "
                            "High-Bitrate Upscale: AI upscaling optimized for already-clean sources (ProRes, high-quality H.265). "
                            "Denoise and Deblur ignore the resize setting and process at input resolution.",
                ),
                io.DynamicCombo.Input(
                    "resize_type",
                    tooltip="Choose how to determine the output resolution. Ignored for Denoise and Deblur modes.",
                    options=[
    ...
    def execute(cls, images: torch.Tensor, mode: str, resize_type: ResizeTypedDict, intensity: str) -> io.NodeOutput:
        ...
        if mode in SAME_RESOLUTION_MODES:
            output_width = w
            output_height = h
        else:
